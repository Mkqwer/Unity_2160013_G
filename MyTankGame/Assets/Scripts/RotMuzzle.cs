...
public class RotMuzzle : NetworkBehaviour
{
    ...
    float bombYellowDelay = 5.0f; // 딜레이(초)
    float lastBombYellowTime = -999f;
    ...
    void Update()
    {
        ...
        {
            if (Time.time - lastBombYellowTime >= bombYellowDelay)
            {
                mBombRpc();
                ExplosionRpc();
    ...
    void mBombRpc()
    {
        GameObject prefabFile = Resources.Load("BombYellow") as GameObject;
        // prefab의 예시 만들기(instantiate)
        GameObject prefab = Instantiate(prefabFile);
        prefab.transform.position = transform.position + transform.forward * 5.7f + transform.up * 0.0f; // transform은 플레이어의 변환
        prefab.transform.rotation = transform.rotation;
